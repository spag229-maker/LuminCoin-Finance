...
export class Expenses {
    ...
    constructor() {
        this.service = new CategoriesService('expense');
    ...
    private showCreateForm(): void {
        if (this.title) this.title.textContent = 'Создание категории расходов';
    ...
    private hideCreateForm(): void {
        if (this.title) this.title.textContent = 'Расходы';
    ...
    private showEditForm(id: number, currentTitle: string): void {
        ...
        if (this.title) this.title.textContent = 'Редактирование категории расходов';
